...
# Modification by Sabina so the module can find its own data after being
# imported as part of a package
import os

DIR = "{}{}".format(pkgdir, "pyspan/ngrams/")
...
def test(verbose=None):
    ...
    import doctest
    print 'Running tests...'
    doctest.testfile(DIR + 'ngrams-test.txt', verbose=verbose)
...
class Pdist(dict):
    ...
    def __init__(self, data=[], N=None, missingfn=None):
        ...
        self.missingfn = missingfn or (lambda k, N, v: 1./N)
    def __call__(self, key, verbose = True):
        if key in self: return self[key]/self.N
        else: return self.missingfn(key, self.N, verbose)
...
def avoid_long_words(key, N, verbose = True):
    "Estimate the probability of an unknown word."
    if verbose:
        print "Encountering unknown word: " + key
    return 10./(N * 10**len(key))

N = 1024908267229 ## Number of tokens

Pw  = Pdist(datafile(DIR + 'count_1w.txt'), N, avoid_long_words)
...
P2w = Pdist(datafile(DIR + 'count_2w.txt'), N)
...
P3l = Pdist(datafile(DIR + 'count_3l.txt'))
P2l = Pdist(datafile(DIR + 'count_2l.txt')) ## We'll need it later
...
P1edit = Pdist(datafile(DIR + 'count_1edit.txt')) ## Probabilities of single edits
